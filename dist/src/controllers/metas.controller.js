...
exports.metasController = exports.MetasController = void 0;
const metas_service_1 = require("../services/metas.service");
const metas_dto_1 = require("../dtos/metas.dto");
...
class MetasController {
    ...
    async getById(req, res, next) {
        try {
            ...
            if (isNaN(metaId)) {
                throw new errors_1.BadRequestError("ID de meta inválido");
            }
            // Llamar al servicio para encontrar la meta
            const meta = await metas_service_1.metasService.findById(metaId);
            // Formatear respuesta usando DTO
            const response = (0, metas_dto_1.createMetaResponse)(meta);
    ...
    async getAll(req, res, next) {
        try {
            ...
            const queryParams = {
                ...
                ...(req.query.desde && { desde: req.query.desde }),
                ...(req.query.hasta && { hasta: req.query.hasta }),
                ...(req.query.activa !== undefined && {
                    activa: req.query.activa === "true",
                }),
                pagina: req.query.pagina
                    ? parseInt(req.query.pagina, 10)
                    : 1,
                tamanoPagina: req.query.tamanoPagina
                    ? parseInt(req.query.tamanoPagina, 10)
                    : 20,
                ...(req.query.orden && { orden: req.query.orden }),
            };
            // Llamar al servicio para obtener las metas
            const result = await metas_service_1.metasService.findAll(queryParams);
            // Formatear respuesta usando DTO
            const response = (0, metas_dto_1.createMetasResponse)(result.metas, {
    ...
    async create(req, res, next) {
        try {
            ...
            const metaId = await metas_service_1.metasService.create(metaData);
            // Formatear respuesta usando DTO
            const response = (0, metas_dto_1.createCreateMetaResponse)(metaId);
    ...
    async update(req, res, next) {
        try {
            ...
            const actualizado = await metas_service_1.metasService.update(metaId, updateData, isAdmin);
            // Formatear respuesta usando DTO
            const response = (0, metas_dto_1.createUpdateMetaResponse)(actualizado);
            // Enviar respuesta exitosa
            res.status(200).json(response);
        }
        catch (error) {
    ...
    async delete(req, res, next) {
        try {
            ...
            const eliminado = await metas_service_1.metasService.delete(metaId, usuarioId);
            // Formatear respuesta usando DTO
            const response = (0, metas_dto_1.createDeleteMetaResponse)(eliminado);
            // Enviar respuesta exitosa
            res.status(200).json(response);
        }
        catch (error) {
 ...
 * ```
 */
exports.metasController = new MetasController();
