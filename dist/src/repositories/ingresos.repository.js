...
class IngresosRepository {
    ...
    async listar(query, usuarioIdResuelto) {
        if (db_1.db.enabled && db_1.db.pool) {
            try {
                const { desde, hasta, tipoId, procedenciaId, min, max, pagina, tamanoPagina, orden, } = query;
                const resultSets = await db_1.db.call("sp_ingresos_listar", [
                    usuarioIdResuelto,
                    desde || null,
                    hasta || null,
                    tipoId || null,
        ...
        let filtered = mockIngresos.filter((ingreso) => ingreso.usuarioId === usuarioIdResuelto);
        if (query.tipoId) {
            filtered = filtered.filter((ingreso) => ingreso.tipoId === query.tipoId);
        }
        if (query.procedenciaId) {
            filtered = filtered.filter((ingreso) => ingreso.procedenciaId === query.procedenciaId);
        }
        if (query.desde) {
            filtered = filtered.filter((ingreso) => new Date(ingreso.fechaInicio) >= new Date(query.desde));
        }
        if (query.hasta) {
            filtered = filtered.filter((ingreso) => new Date(ingreso.fechaFin ?? ingreso.fechaInicio) <= new Date(query.hasta));
        }
        ...
        filtered = filtered.sort((a, b) => {
            ...
            if (campo === "fechaInicio") {
                return ((new Date(a.fechaInicio).getTime() -
    ...
    async crear(body, usuarioIdResuelto) {
        if (db_1.db.enabled && db_1.db.pool) {
            try {
                const resultSets = await db_1.db.call("sp_ingresos_crear", [
                    usuarioIdResuelto,
                    body.tipoId,
                ...
                if (error.message && error.message.includes("FK_INEXISTENTE")) {
                    throw new Error("FK_INEXISTENTE: tipoId o procedenciaId no existe");
                }
            }
        }
        return Math.floor(Math.random() * 1000) + 100;
    }
    async obtener(ingresoId) {
        if (db_1.db.enabled && db_1.db.pool) {
            try {
                const resultSets = await db_1.db.call("sp_ingresos_obtener", [ingresoId]);
                const rows = resultSets[0];
    ...
    async actualizar(ingresoId, body, usuarioIdResuelto) {
        if (db_1.db.enabled && db_1.db.pool) {
            try {
                ...
                const resultSets = await db_1.db.call("sp_ingresos_actualizar", [
                    ingresoId,
                    usuarioIdResuelto,
    ...
    async eliminar(ingresoId, usuarioIdResuelto) {
        if (db_1.db.enabled && db_1.db.pool) {
            try {
                const resultSets = await db_1.db.call("sp_ingresos_eliminar", [
                    ingresoId,
                    usuarioIdResuelto,
                ]);
                const rows = resultSets[0];
...
exports.ingresosRepository = new IngresosRepository();
