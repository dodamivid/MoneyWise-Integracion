...
class UserService {
    ...
    async findById(id) {
        ...
        if (!user) {
            throw new errors_1.NotFoundError("Usuario", id.toString());
        }
        return user;
    }
    /**
    ...
    async actualizarPerfil(id, updateData) {
        ...
        if (Object.keys(updateData).length === 0) {
            throw new errors_1.ValidationError("Debe proporcionar al menos un campo para actualizar");
        }
     ...
     * ```
     */
    async cambiarContrasena(id, passwordData) {
        // Validar formato del ID
        try {
            user_model_1.UserIdSchema.parse(id);
        }
        ...
        try {
            user_model_1.ChangePasswordSchema.parse(passwordData);
        }
        catch (error) {
            throw new errors_1.ValidationError(error.errors?.map((e) => e.message).join(", ") ||
        ...
        const passwordMatches = await bcrypt_1.default.compare(passwordData.contrasenaActual, existingUser.contrasena);
        if (!passwordMatches) {
            // Usar BadRequestError con código 401 para contraseña incorrecta
            const error = new errors_1.BadRequestError("La contraseña actual es incorrecta");
            error.statusCode = 401; // Override para usar 401 en lugar de 400
            throw error;
        }
        // Hashear la nueva contraseña
        const hashedPassword = await bcrypt_1.default.hash(passwordData.contrasenaNueva, BCRYPT_SALT_ROUNDS);
        // Actualizar la contraseña
        await user_repository_1.userRepository.update(id, {
