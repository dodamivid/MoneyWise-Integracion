...
exports.metasService = exports.MetasService = void 0;
const metas_repository_1 = require("../repositories/metas.repository");
...
class MetasService {
     ...
     * ```
     */
    async findById(metaId) {
        // Validar formato del ID
        try {
            meta_model_1.MetaIdSchema.parse(metaId);
        }
        catch (error) {
            throw new errors_1.ValidationError(`Formato de ID de meta inválido: ${metaId}`);
        }
        // Intentar encontrar la meta
        const meta = await metas_repository_1.metasRepository.findById(metaId);
        // Lanzar error si no se encuentra
        if (!meta) {
            throw new errors_1.NotFoundError("Meta", metaId.toString());
        }
        return meta;
    }
    /**
    ...
    async findAll(queryParams) {
        ...
        let validatedParams;
        try {
            validatedParams = meta_model_1.MetaQueryParamsSchema.parse(queryParams);
        }
        catch (error) {
        ...
        return metas_repository_1.metasRepository.findAll(filters, pagination);
    }
    /**
    ...
    async findByUsuarioId(usuarioId) {
        if (!Number.isInteger(usuarioId) || usuarioId <= 0) {
            throw new errors_1.ValidationError("El ID de usuario debe ser un número entero positivo");
        }
        return metas_repository_1.metasRepository.findByUsuarioId(usuarioId);
    }
    /**
    ...
    async create(metaData) {
        ...
        try {
            meta_model_1.CreateMetaSchema.parse(metaData);
        }
        catch (error) {
            throw new errors_1.ValidationError(error.errors?.map((e) => e.message).join(", ") ||
        ...
        if (metaData.montoObjetivo <= 0) {
            throw new errors_1.BadRequestError("El monto objetivo debe ser mayor a 0");
        }
        if (metaData.fechaFin) {
            const fechaInicio = new Date(metaData.fechaInicio);
            const fechaFin = new Date(metaData.fechaFin);
            if (fechaFin < fechaInicio) {
                throw new errors_1.BadRequestError("La fecha de fin debe ser mayor o igual a la fecha de inicio");
            }
        }
        // Crear meta
        const meta = await metas_repository_1.metasRepository.create(metaData);
        return meta.metaId;
    }
    /**
     ...
     * ```
     */
    async update(metaId, updateData, isAdmin = false) {
        // Validar formato del ID
        try {
            meta_model_1.MetaIdSchema.parse(metaId);
        }
        ...
        try {
            meta_model_1.UpdateMetaSchema.parse(updateData);
        }
        catch (error) {
            throw new errors_1.ValidationError(error.errors?.map((e) => e.message).join(", ") ||
        ...
        const existingMeta = await metas_repository_1.metasRepository.findById(metaId);
        if (!existingMeta) {
            throw new errors_1.NotFoundError("Meta", metaId.toString());
        }
        ...
        const newMontoObjetivo = updateData.montoObjetivo ?? existingMeta.montoObjetivo;
        if (!isAdmin && newAhorroReal > newMontoObjetivo) {
            throw new errors_1.BadRequestError("El ahorro real no puede superar el monto objetivo sin permisos de administrador");
        }
        ...
        if (updateData.ahorroReal !== undefined && updateData.ahorroReal < 0) {
            throw new errors_1.BadRequestError("El ahorro real no puede ser negativo");
        }
        ...
        // Actualizar meta
        const updatedMeta = await metas_repository_1.metasRepository.update(metaId, updateData);
    ...
    async delete(metaId, usuarioId) {
        ...
        if (existingMeta.usuarioId !== usuarioId) {
            throw new errors_1.BadRequestError("No tiene permisos para eliminar esta meta");
        }
        // Intentar eliminar (soft delete)
        const deleted = await metas_repository_1.metasRepository.delete(metaId);
        if (!deleted) {
            throw new errors_1.NotFoundError("Meta", metaId.toString());
        }
        return deleted;
    ...
    async exists(metaId) {
        ...
        return metas_repository_1.metasRepository.exists(metaId);
    }
    /**
     ...
     */
    async count() {
        return metas_repository_1.metasRepository.count();
    }
    /**
     ...
     * ```
     */
    async countActiveByUsuario(usuarioId) {
        if (!Number.isInteger(usuarioId) || usuarioId <= 0) {
            throw new errors_1.ValidationError("El ID de usuario debe ser un número entero positivo");
        }
        return metas_repository_1.metasRepository.countActiveByUsuario(usuarioId);
    }
}
 ...
 * ```
 */
exports.metasService = new MetasService();
