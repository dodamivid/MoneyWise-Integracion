...
class EgresosRepository {
  ...
  async listar(
    ...
    if (query.desde) {
      filtered = filtered.filter(
        (e) => new Date(e.fechaInicio) >= new Date(query.desde!)
      );
    }
    if (query.hasta) {
      filtered = filtered.filter(
        (e) => new Date(e.fechaInicio) <= new Date(query.hasta!)
      );
    }
   ...
   */
  async obtener(egresoId: number): Promise<EgresoDTO | null> {
    if (db.enabled && db.pool) {
      try {
        const resultSets = await db.call("sp_egresos_obtener", [egresoId]);
        const rows = resultSets[0] as any[];
  ...
  async actualizar(
    ...
    if (db.enabled && db.pool) {
      try {
        ...
        const resultSets = await db.call("sp_egresos_actualizar", [
          egresoId,
          usuarioIdResuelto,
  ...
  async eliminar(
    ...
    if (db.enabled && db.pool) {
      try {
        const resultSets = await db.call("sp_egresos_eliminar", [
          egresoId,
          usuarioIdResuelto,
        ]);
