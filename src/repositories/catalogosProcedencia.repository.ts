...
export class CatalogosProcedenciaRepository {
  private procedencias: ProcedenciaDTO[] = [];
  ...
  async listarProcedencias(
    ...
    if (db.enabled && db.pool) {
      const resultSets = await db.call("sp_procedencias_listar", [
        usuarioId,
        buscar ?? "",
        pagina,
        tamanoPagina,
    ...
    const disponibles = this.procedencias.filter(
      (p) => p.usuarioId === null || p.usuarioId === usuarioId
    );
  ...
  async crearProcedencia(
    ...
    if (db.enabled && db.pool) {
      const resultSets = await db.call("sp_procedencias_crear", [
        usuarioId,
        nombre,
      ]);
    ...
    const existe = this.procedencias.find(
    ...
    const procedenciaId =
      Math.max(...this.procedencias.map((p) => p.procedenciaId), 0) + 1;
    const now = new Date().toISOString();
  ...
  async actualizarProcedencia(
    ...
    if (db.enabled && db.pool) {
      const resultSets = await db.call("sp_procedencias_actualizar", [
        procedenciaId,
        usuarioId,
        nombre,
      ]);
    ...
    const procedencia = this.procedencias.find(
      (p) => p.procedenciaId === procedenciaId
    );
    if (!procedencia) {
      throw new Error("NO_ENCONTRADO");
    }
    ...
    const duplicado = this.procedencias.find(
      (p) =>
        p.procedenciaId !== procedenciaId &&
  ...
  async eliminarProcedencia(
    ...
    if (db.enabled && db.pool) {
      const resultSets = await db.call("sp_procedencias_eliminar", [
        procedenciaId,
        usuarioId,
      ]);
    ...
    this.procedencias = this.procedencias.filter(
      (p) => p.procedenciaId !== procedenciaId
    );
...
export const catalogosProcedenciaRepository =
  new CatalogosProcedenciaRepository();
