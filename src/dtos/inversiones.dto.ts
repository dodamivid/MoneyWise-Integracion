...
const positiveInt = z.coerce.number().int("Debe ser un entero").positive("Debe ser positivo");

const destinoIdNullable = z.union([
  z.coerce.number().int("destinoId debe ser entero").positive("destinoId debe ser positivo"),
  z.null(),
]);
...
const fechaSchema = z.string().datetime("Formato de fecha inválido");
...
const montoSchema = z.coerce.number().gt(0, "El monto debe ser mayor a 0");

const tasaSchema = z.coerce.number().min(0, "La tasa debe ser mayor o igual a 0").max(100, "La tasa no puede superar 100");

const ordenRegex = /^(fechaInicio|fechaFin|monto|creadoEn)(?::(asc|desc))?$/i;

const paginaSchema = z.coerce.number().int("La página debe ser entera").min(1, "La página debe ser al menos 1").default(1);
